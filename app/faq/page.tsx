...
function useTokens() {
  ...
  return {
    ...
    sectionBorder: isDark ? "rgba(249,245,236,0.1)"    : "var(--card-border)",
    subtitle:      isDark ? "rgba(249,245,236,0.92)"   : "rgba(27,42,71,0.72)",
    cardBg:        isDark ? "rgba(249,245,236,0.04)"   : "var(--card-bg)",
    cardBorder:    isDark ? "rgba(249,245,236,0.12)"   : "var(--card-border)",
    question:      isDark ? "rgba(249,245,236,0.92)"   : "var(--text)",
    answer:        isDark ? "rgba(249,245,236,0.72)"   : "rgba(27,42,71,0.65)",
    chevron:       isDark ? "rgba(249,245,236,0.72)"   : "rgba(27,42,71,0.45)",
...
const FAQAccordion = () => {
  ...
  useGSAP(
    () => {
      ...
      if (reduce) {
        gsap.set([introEl, accordionEl], { clearProps: "all", autoAlpha: 1 });
        return;
      }

      // Animation texte intro mot par mot (opacité uniquement)
      (async () => {
        ...
        const toSplit = targets.filter((el) => !el.classList.contains("splitting"));
        ...
        gsap.set(words, { opacity: 0.15, willChange: "opacity" });
        gsap.to(words, {
          opacity: 1,
          duration: 1,
          ease: "none",
      ...
      const faqItems = accordionEl.querySelectorAll(".faq-accordion-item");
  ...
  return (
    ...
    <section
      id="faq"
      ref={sectionRef}
      className="relative overflow-hidden min-h-screen border-b transition-colors duration-500"
      style={{
          ...
          <Accordion.Root type="single" collapsible className="space-y-4">
            {faqData.map((item) => (
              <Accordion.Item
                key={item.id}
                value={item.id}
                className="faq-accordion-item rounded-xl overflow-hidden transition-all duration-300"
                style={{
                  background:  t.cardBg,
                  borderWidth: "1px",
                  borderStyle: "solid",
              ...
              >
                <Accordion.Trigger className="group w-full flex items-center justify-between px-6 py-5 text-left transition-all hover:border-[rgba(174,137,74,0.35)]">
                  <span
                    className="text-base sm:text-lg font-semibold pr-4 group-hover:text-[rgba(174,137,74,1)] transition-colors duration-200"
                    style={{ color: t.question }}
                  >
                    {item.question}
                  </span>
                  <motion.div
                    className="shrink-0 group-hover:text-[rgba(174,137,74,1)] transition-colors duration-200"
                ...
                <Accordion.Content className="overflow-hidden data-[state=open]:animate-slideDown data-[state=closed]:animate-slideUp">
                  <div
                    className="px-6 pb-5 pt-2 text-sm sm:text-base leading-relaxed"
                    style={{ color: t.answer }}
                  >
                    {item.answer}
                  </div>
                </Accordion.Content>
